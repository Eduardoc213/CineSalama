...
// useParams es para leer el [token] de la URL
...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:3000/api/auth/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Enviamos el token de la URL y la nueva contraseña
  ...
  // Si ya fue exitoso, muestra un mensaje
  if (success) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50 p-8">
        <div className="max-w-md w-full text-center bg-white p-8 shadow-md rounded-lg">
          <h1 className="text-3xl font-bold text-green-600">¡Éxito!</h1>
