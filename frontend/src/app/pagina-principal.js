...
export default function PaginaPrincipal() {
  ...
  const [salasMap, setSalasMap] = useState({}); // id -> sala
  ...
  // helper para obtener pelicula por id
  ...
  // -------- Lo más visto: dos películas con más funciones ----------
  ...
  // -------- Cartelera: funciones durante próximos 7 días (desde hoy) ----------
  const cartelera = useMemo(() => {
    if (!funciones || funciones.length === 0) return [];
    const now = new Date();
    const start = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0); // hoy 00:00
    const end = new Date(start.getTime() + 7 * 24 * 60 * 60 * 1000); // +7 días
  ...
  // -------- Próximos estrenos: solo estrenos futuros (fecha_estreno > ahora) ----------
